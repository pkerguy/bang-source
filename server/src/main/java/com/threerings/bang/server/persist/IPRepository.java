 ...
 */
@Singleton
public class IPRepository extends JORARepository
{
     ...
     */
    @Inject public IPRepository(ConnectionProvider conprov)
        throws PersistenceException
    {
        super(conprov, PLAYER_DB_IDENT);
        _byNameMask = _ptable.getFieldMask();
        _byNameMask.setModified("username");
    }

    /**
     * Loads up the IP record associated with the specified account.  Returns null if no
     * matching record could be found.
     */
    public IPRecord loadIP (String accountName, String IP)
        throws PersistenceException
    {
        return loadByExample(_ptable, new IPRecord(accountName, IP), _byNameMask);
    }

    /**
     * Loads up the IP record based on IP address
     */
    public IPRecord loadByIP (String IP)
        throws PersistenceException
    {
        return loadIP(_ptable, "where IP = " + IP);
    }
    ...
    protected Table<IPRecord> _ptable;
