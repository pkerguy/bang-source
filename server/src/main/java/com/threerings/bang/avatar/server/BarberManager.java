    ...
    implements BarberCodes, BarberProvider, AvatarProvider
{
        ...
        throws InvocationException
    {
        ...
        if (!NameFactory.getValidator().isValidHandle(handle)) {
            BangServer.DISCORD.commit(1, "User tried to use invalid handle", "who", user.who(), "handle", handle);
        ...
        throws InvocationException
    {
        ...
        if (StringUtil.isBlank(config.name) ||
            ...
            BangServer.DISCORD.commit(1, "Requested to create look with blank name", "who", user.who());
            throw new InvocationException(INTERNAL_ERROR);
        }
        ...
        if (current != null) {
            ...
            BangServer.DISCORD.commit(1, "Player has no current look from which to copy articles",
    ...
    public void configureLook (PlayerObject user, String name, int[] articles)
    {
        ...
        if (look == null) {
            BangServer.DISCORD.commit(1, "Asked to configure unknown look", "who", user.who(), "look", name,
        ...
        if (articles == null || articles.length != AvatarLogic.SLOTS.length) {
            BangServer.DISCORD.commit(1, "Requested to configure invalid articles array", "who", user.who(),
        ...
        for (int ii = 0; ii < articles.length; ii++) {
            if (articles[ii] == 0) {
                ...
                BangServer.DISCORD.commit(1, "Requested to configure look with missing non-optional articles",
            ...
            if (article == null || !article.isWearable(user) ||
                ...
                BangServer.DISCORD.commit(1, "Asked to configure look with invalid article", "who", user.who(),
    ...
    public void selectLook (PlayerObject user, Look.Pose pose, String name)
    {
        ...
        if (look == null) {
            BangServer.DISCORD.commit(1, "Player requested to select unknown look", "who", user.who(), "look", name);
            return;
        }
